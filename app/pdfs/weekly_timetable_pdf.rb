  ...
  def table_schedule_sun_wed
    ...
    @weeklytimetable.timetable_monthurs.timetable_periods.order(sequence: :asc).in_groups_of(@count1, false).map do |row_things|
	  ...
	  if @break_format1[col-1]==true && row==1 
		...
		gg+="#{xx.location_desc}#{"(K)" if xx.lecture_method==1} #{"(T)" if xx.lecture_method==2}#{"(A)" if xx.lecture_method==3} #{@college.code=="amsas" ? xx.weeklytimetable_lecturer.staff_with_rank : xx.weeklytimetable_lecturer.name}"
	      end 
	    end
	    onerow_content << gg
          end
