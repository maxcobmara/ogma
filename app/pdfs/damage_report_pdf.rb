class Damage_reportPdf < Prawn::Document 
  ...
  def line_item_rows
    ...
    header +
      ...
      ["#{counter += 1}", "#{damage.try(:location).try(:combo_code)}", "#{damage.damage_type} " , "#{damage.description}"," #{damage.reported_on.try(:strftime, '%d-%m-%Y')}", "#{damage.repaired_on.try(:strftime, '%d-%m-%Y')}",  "#{damage.try(:tenant).try(:student).try(:name)}"]
    end
  end
