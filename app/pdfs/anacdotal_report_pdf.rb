class Anacdotal_reportPdf < Prawn::Document 
  def initialize(discipline_cases, view)
    ...
    text "#{I18n.t('student.students.intake_id')} : #{@discipline_cases.first.student.intake.try(:strftime, '%b %Y')}", :align => :left, :size => 11, :indent_paragraphs => 10
    move_down 10
    record
  end
