class Kewpa17Pdf < Prawn::Document
  ...
  def line_item_rows
      ...
      header +
        ...
        ["#{counter += 1}", "", "#{disposal.try(:asset).try(:assetcode)} #{disposal.try(:asset).try(:name)}", "" , "#{disposal.try(:asset).try(:quantity)}","#{disposal.try(:asset).try(:purchasedate).try(:strftime, "%d/%m/%y")}", 
