class Damage_report_staffPdf < Prawn::Document 
  def initialize(damages, view, college)
    ...
    text "#{I18n.t('location.damage.damage_report_staff')}", :align => :center, :size => 11, :style => :bold
    move_down 10
    record
  end
  
  def record
    table(line_item_rows, :column_widths => [30,70, 75, 155, 60, 60, 70], :cell_style => { :size => 9,  :inline_format => :true}) do
  ...
  def line_item_rows
    ...
    header +
      @damages.sort_by{|x|x.location.combo_code}.map do |damage|
