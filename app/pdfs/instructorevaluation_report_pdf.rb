class Instructorevaluation_reportPdf < Prawn::Document  
  def initialize(instructor_appraisals, view, college)
    ...
    bounding_box([150,750], :width => 350, :height => 100) do |y2|
      if college.code=="kskbjb"
        ...
        draw_text "PUSAT LATIHAN DAN AKADEMI MARITIM MALAYSIA", :at => [-20, 85], :style => :bold, :size => 11
        draw_text "(PLAMM)", :at => [85, 70], :style => :bold, :size => 11
        draw_text "#{I18n.t('instructor_appraisal.report').upcase}", :at => [20, 45], :style => :bold,  :size => 11
      end
    end
  ...
  def table_report
    ...
    data=header+body
    table(data, :column_widths => [30, 200, 80, 80, 130], :cell_style => {:size=>10, :borders => [:left, :right, :top, :bottom]}) do
      a = 0
      b = 1
