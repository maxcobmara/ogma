class Ptdosearch < ActiveRecord::Base
  ...
  def set_staff_data
    ...
    if !schedulestart_start.blank? && schedulestart_end.blank?
      self. schedulestart_end=Date.today
    end
  end
...
#   end
  ...
  def staff_name_conditions
    if !staff_name.blank? && department.blank?
      staff_ids = Staff.where('name ILIKE (?)', "%#{staff_name}%").map(&:id)
      a="staff_id=?" if staff_ids.count > 0
      0.upto(staff_ids.count-2) do |x|
        a+=" OR staff_id=? "
      end
  ...
  def icno_conditions
    if !icno.blank? #&& department.blank?
      staff_ids = Staff.where('icno ILIKE (?)', "%#{icno}%").map(&:id)
      a="staff_id=?" if staff_ids.count > 0
      0.upto(staff_ids.count-2) do |x|
        a+=" OR staff_id=? "
      end
  ...
  def schedulestart_start_conditions
    unless schedulestart_start.blank?
      ptschedule_ids = Ptschedule.where('start >=?', schedulestart_start).map(&:id)
      ptdo_ids = Ptdo.where('ptschedule_id IN(?)', ptschedule_ids).map(&:id)
  ...
  def schedulestart_end_conditions
    unless schedulestart_end.blank?
      ptschedule_ids = Ptschedule.where('start <=?', schedulestart_end).map(&:id)
      ptdo_ids = Ptdo.where('ptschedule_id IN(?)', ptschedule_ids).map(&:id)
      ptdo_ids2 = Ptdo.all.map(&:id)
      a="id=?" if ptdo_ids.count > 0
      0.upto(ptdo_ids.count-2) do |x|
