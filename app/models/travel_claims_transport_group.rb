  ...
  def self.transport_class(car_id, current_salary, abc_rate, de_rate, mid_salary)
    ...
    #group A, B or C (order - asc)
      ...
      #by cc
       for ascl in abc_rate
        if cnt2==0
          if ascl.cc_low && (Vehicle.where(id: car_id)[0].cylinder_capacity>=ascl.cc_low)		#group A or B
            group_cc = ascl.group_name
            cnt2+=1 
          elsif ascl.cc_high && (Vehicle.where(id: car_id)[0].cylinder_capacity<=ascl.cc_high)  #group C
            group_cc = ascl.group_name
          end
        end
       end
     ...
     #group E or D (order - desc)
       for asch in de_rate
         if asch.cc_high && (Vehicle.where(id: car_id)[0].cylinder_capacity <= asch.cc_high)	#group E
           group_de = asch.group_name
         elsif asch.cc_low && (Vehicle.where(id: car_id)[0].cylinder_capacity >= asch.cc_low)	#group D
           group_de = asch.group_name
         end
       end
       group = group_de
