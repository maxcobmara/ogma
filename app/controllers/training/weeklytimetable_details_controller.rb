class WeeklytimetableDetailsController < ApplicationController
  before_action :set_weeklytimetable_detail, only: [:show, :edit, :update, :destroy]
...
#      flash[:error] => 'Removal of Weeklytimetable Details is forbidden, due to existance in Student Attendance module.'
  ...
  private
    ...
    def set_weeklytimetable_detail
      @weeklytimetable = Weeklytimetable.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def weeklytimetable_detail_params
      params.require(:weeklytimetable_detail).permit(:topic, :time_slot, :lecturer_id, :weeklytimetable_id, :day2, :is_friday, :time_slot2, :location, :lecture_method, :location_desc)
    end
end
