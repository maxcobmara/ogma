class Library::BooksController < ApplicationController
  ...
  def stock_listing
    respond_to do |format|
      format.pdf do
         pdf = Stock_listingPdf.new(@all_accessions, @search, @params_count, view_context, "Kolej Sains Kesihatan Bersekutu Johor Bahru")
  ...
  def book_summary
    respond_to do |format|
      format.pdf do
         pdf = Book_summaryPdf.new(@all_accessions, @search, @params_count, view_context, "Kolej Sains Kesihatan Bersekutu Johor Bahru")
         send_data pdf.render, filename: "document_report-{Date.today}",
                               type: "application/pdf",
                               disposition: "inline"
      end
