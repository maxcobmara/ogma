class Staff::StaffAppraisalsController < ApplicationController
  ...
  private
    ...
    def staff_appraisal_params
      ...
      staff_appraisal_skts_attributes: [:id,:_destroy, :priority, :description, :half, :indicator, :acheivment, :progress, :notes, :is_dropped, :dropped_on, :drop_reasons], evactivities_attributes: [:id,:_destroy, :evaldt, :evactivity, :actlevel, :actdt], trainneeds_attributes: [:id,:_destroy, :name, :reason, :confirmedby_id, :evaluation_id])
    end
